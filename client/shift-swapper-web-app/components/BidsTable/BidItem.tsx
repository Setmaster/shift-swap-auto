...
function getBidInfo(bid: Bid, theme: MantineTheme, isHighestBid: boolean) {
    if (isHighestBid) {
        return {
    ...
    const bidInfo = statusInfo[bid.bidStatus as BidStatus] || statusInfo['Other'];

    return bidInfo;
}

export default function BidItem({bid, isHighestBid}: { bid: Bid, isHighestBid: boolean }) {
    const theme = useMantineTheme();
    const bidInfo = getBidInfo(bid, theme, isHighestBid);
